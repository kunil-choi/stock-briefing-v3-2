...
트리거합니다. (`stock-briefing-step2` 자동 트리거는 과금 방지를 위해 제거했습니다 —
아래 트리거 체인 참고.)
...
`stock-briefing-step2`(report_update.yml, 실제 영상 생성 — OpenAI/TTS 과금 발생)은
더 이상 자동 트리거하지 않습니다(과금 방지를 위한 의도적 변경). `docs/index.html`로
데이터를 사람이 먼저 확인한 뒤, step2 저장소 Actions 탭에서 수동으로
`workflow_dispatch`를 실행하세요.

자체 cron은 두지 않습니다(`stock-briefing-video`의 `daily_broadcast.yml` 설계
원칙과 동일 — 상류가 끝나기 전에 실행되면 날짜 불일치 위험).
